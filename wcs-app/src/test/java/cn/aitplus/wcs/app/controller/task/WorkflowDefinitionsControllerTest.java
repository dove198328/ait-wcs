...
public class WorkflowDefinitionsControllerTest {
    ...
    void queryList() {
        Integer pageNum = 1;
        Integer pageSize = 10;
//        String testBizType = "RK1";
        String baseUrl = "/api/" + wareHouseId + "/workflow-definitions/search";
        WorkflowDefinition query = new WorkflowDefinition();
        query.setWarehouseId(wareHouseId.intValue());
//        query.setBizType(testBizType);
        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.APPLICATION_JSON);
        HttpEntity<WorkflowDefinition> entity = new HttpEntity<>(query, headers);
        // ✅ 分页查询（可控参数）
        String urlPage = baseUrl + "?pageNum=" + pageNum + "&pageSize=" + pageSize;
        ResponseEntity<String> pageResp =
                restTemplate.postForEntity(urlPage, entity, String.class);
        System.out.println("=== 分页查询 ===");
        System.out.println(pageResp.getBody());
        // ✅ 不分页查询（同样用手动条件）
        ResponseEntity<String> noPageResp =
                restTemplate.postForEntity(baseUrl, entity, String.class);

        System.out.println("=== 不分页查询 ===");
        System.out.println(noPageResp.getBody());
    }

    @Test
    @Order(2)
    @DisplayName("新增流程定义并部署到工作流")
    void create() {
        // 👉 手动控制输入数据
        String testName = "测试流程";
        String testWorkflowId = "Process_1763640437532";
        String testBizType = "TEST_CREATE";
        String testConfig = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" +
                ...
                "  </bpmndi:BPMNDiagram>\n" +
                "</bpmn:definitions>\n";
        Long testWarehouseId = wareHouseId;
        String url = "/api/" + testWarehouseId + "/workflow-definitions";
        // 👉 构造请求参数
        WorkflowDefinition req = new WorkflowDefinition();
        req.setName(testName);
        req.setWorkflowId(testWorkflowId);
        req.setBizType(testBizType);
        req.setConfig(testConfig);
        req.setWarehouseId(testWarehouseId.intValue());
        req.setCreatedAt(LocalDateTime.now());
        req.setUpdatedAt(LocalDateTime.now());
        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.APPLICATION_JSON);
        HttpEntity<WorkflowDefinition> entity = new HttpEntity<>(req, headers);
        // 👉 调用接口
        ResponseEntity<String> response =
                restTemplate.postForEntity(url, entity, String.class);

        System.out.println("=== 新增流程 ===");
        ...
        try {
            ...
            if (data != null && data.has("id")) {
                createdId = data.get("id").asLong();
                System.out.println("获取到ID: " + createdId);
            } else {
                throw new RuntimeException("创建失败，未返回ID");
            }
        } catch (Exception e) {
            throw new RuntimeException("解析返回值失败", e);
        }
    }

    @Test
    @Order(3)
    @DisplayName("更新流程定义并重新部署到工作流")
    void update() {
        Long createdId  = 145L;
        if (createdId == null) {
            System.out.println("跳过更新：没有创建数据");
            return;
        }

        String url = "/api/" + wareHouseId + "/workflow-definitions/" + createdId;

        WorkflowDefinition req = new WorkflowDefinition();
        req.setName("测试流程-更新");
        req.setBizType("TEST_UPDATE");
        req.setWarehouseId(wareHouseId.intValue());
    ...
    @Test
    @Order(4)
    @DisplayName("物理删除流程定义")
    void delete() {
        Long createdId  = 146L;
        if (createdId == null) {
            System.out.println("跳过删除：没有创建数据");
            return;
        }
        ...
        System.out.println("删除ID: " + createdId);
    }

    @Test
    @Order(5)
    @DisplayName("根据业务类型查询流程定义")
    void findByBizType() {

        String testBizType = "RK1";

        String url = "/api/" + wareHouseId + "/workflow-definitions/biz-type/" + testBizType;

        ResponseEntity<String> response = restTemplate.getForEntity(url, String.class);

        System.out.println("=== bizType查询 ===");
        System.out.println(response.getBody());
    }

    @Test
    @Order(6)
    @DisplayName("根据流程ID查询流程定义")
    void queryByWorkflowId() {
        List<String> workflowIds = Arrays.asList(
                "workflow_1758110275067_2579",
                "workflow_1758176548572_5126"
        );

        for (String workflowId : workflowIds) {

            String url = "/api/" + wareHouseId + "/workflow-definitions/workflow/" + workflowId;

            ResponseEntity<String> response = restTemplate.getForEntity(url, String.class);

            System.out.println("=== workflowId查询: " + workflowId + " ===");
            System.out.println(response.getBody());
        }
    }
    ...
    @Test
    @Order(7)
    @DisplayName("根据流程ID查询首个子流程定义")
    void getFirstSubDefString() {
        List<String> workflowIds = Arrays.asList(
                "workflow_1758110275067_2579"
        );
        for (String workflowId : workflowIds) {

            String url = "/api/" + wareHouseId + "/workflow-definitions/workflow/"
                    + workflowId + "/first-sub-def";
            ResponseEntity<String> response = restTemplate.getForEntity(url, String.class);
            System.out.println("=== 子流程查询: " + workflowId + " ===");
            System.out.println(response.getBody());
        }
    }
}